...
for line in lines: #fill list
  rel=line[0]
  relevant.append(rel)
for line in lines:
  args=len(line)
  for arg in range(3,args):
    arg=line[arg];
    #print arg
    relevant.remove(arg)
# print relevant #part 1 answer

#there is a problem: how do we know which disc is the one we have to chage,
#if there are two discs on a specific tower?
weights=dict()
while (8):
  for line in lines:
    args=len(line)
    name=line[0];
    if not (weights.has_key(name)): #don't check if already taken care of
      weight=int(line[1][1:-1])
      if (args < 3): #leaf case, all checked after first iteration
        pass;
      else: #branch case
        for arg in range(3,args):
          argName=line[arg];
          if (weights.has_key(argName)):
            if (arg==3):
              firstWeight=weights[argName]
            else:
              if not (weights[argName]==firstWeight): #convict found
                print name, "looks like this:\n", line, \
                "\nBut the weights are:"
                for arg in range (3,args):
                  print line[arg],
                  if (weights.has_key(line[arg])):
                    print ":",weights[line[arg]];
                  else:
                    print ": unknown"
                exit() 
            weight+=weights[argName];
          else: #some sub-tower has no weight yet
            weight=0;
            break;
      if (weight > 0): #if weight was found correctly, add to weights
        weights[name]=weight;
#ycbgx's weight is 1531, so the correct weight is 1526.