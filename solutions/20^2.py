...
try:
  ...
  #fix this
    dups=set()
    coWithIndex=[b+[a] for a,b in zip(co.keys(),co.values())]
    sortedCo=sorted(coWithIndex,key=op.itemgetter(0,1,2))
    length=len(sortedCo)
    for i in range(length-1):
      if (sortedCo[i][0]==sortedCo[i+1][0] and sortedCo[i][1]==sortedCo[i+1][1] and sortedCo[i][2]==sortedCo[i+1][2]):
        dups.add(sortedCo[i][9])
        dups.add(sortedCo[i+1][9])
    return dups
  def progress(co):
    for coo in co.values():
      coo[3]+=coo[6]
      coo[4]+=coo[7]
      coo[5]+=coo[8]
      coo[0]+=coo[3]
  ...
  #main
  input=input.split('\n')
  p="p=<(?P<p1>-?\d*),(?P<p2>-?\d*),(?P<p3>-?\d*)>, v=<(?P<v1>-?\d*),(?P<v2>-?\d*),(?P<v3>-?\d*)>, a=<(?P<a1>-?\d*),(?P<a2>-?\d*),(?P<a3>-?\d*)>";
  c={}
  dups=None;
  #fill coordinates
  for i,j in enumerate(input):
    res=re.search(p,j)
    c[i]=[int(k) for k in list(res.groups())]
    particles=len(c)
  #tick
  for counter in range (1000000):
    #sanity check
    if(not counter%200):
      print counter,"th tick"
    #check dups
    dups=check_dup(c)
    for dup in dups:
      c.pop(dup)
    ...
    if(len(c)<particles):
      particles=len(c)
      print "particles left:",len(c)
    #advance time
    progress(c)
except Exception as e:
  print str(e),exc_info()[2].tb_lineno
finally:
  raw_input()