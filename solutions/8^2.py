...
registers = {}
for inst in instructions: #init registers
  reg = (inst.split(' ')[0])
  registers[reg]=0
#print registers
for inst in instructions:
  ...
  imm2=int(inst[6])
  flag=False;
  #print RS,sign,imm1,RT,symbol,imm2
  if (symbol=='<'):  #Check condition
    if (registers[RT] < imm2):
      flag=True;
  elif (symbol=='<='):
    if (registers[RT] <= imm2):
      flag=True;
    ...
    exit()
  #print flag
  if (flag): #Condition True
    if (sign == 'inc'):
      sign=1;
    else:
      sign=-1;
    registers[RS]+=(imm1*sign)
    if (registers[RS] > maxest):
      maxest=registers[RS];
max=registers.itervalues().next()  #find max
for k,v in registers.iteritems():
  if (v>max):
    max=v;
print "highest right now:",max
print "highest ever:",maxest